    ...
    <body class="bg-custom-green">
        ...
        <div class="flex flex-grow flex-col items-center justify-center p-4">
            <div class="bg-custom-gray rounded-lg p-6 md:p-8 mb-6 form-border text-shadow-black w-full max-w-md">
                <form action="<c:out value="${contextPath}" />/UserController" method="post">
                    <!----------------------------------------------------->
                    <div>
                    ...
                    <input type="text" id="nombre" name="usuario" placeholder="Ejem: José" value="<c:if test="${not empty datosFormulario['nombre']}"><c:out value="${datosFormulario['nombre']}" /></c:if>"
                        ...
                        <input type="text" id="apellidos" name="usuario" placeholder="Apellidos" value="<c:if test="${not empty datosFormulario['apellidos']}"><c:out value="${datosFormulario['apellidos']}" /></c:if>"
                               class="border border-green-600 rounded w-full py-2 px-3 leading-tight focus:outline-none text-shadow-black"
                               required />
                        <!----Email------------------------------------------------->
                        <label for="email" class="block text-green-600 text-lg font-bold mb-2 mt-4">Email:</label>
                        <input type="email" id="email" name="usuario"  placeholder="Email" value="<c:if test="${not empty datosFormulario['email']}"><c:out value="${datosFormulario['email']}" /></c:if>"
                               class="border border-green-600 rounded w-full py-2 px-3 leading-tight focus:outline-none text-shadow-black"
                               required />
                        <!----Password------------------------------------------------->
                        <label for="password" class="block text-green-600 text-lg font-bold mb-2 mt-4">Password :</label>
                        <input type="password" id="password" name="usuario"  placeholder="Password" value="<c:if test="${not empty datosFormulario['password']}"><c:out value="${datosFormulario['password']}" /></c:if>"
                               class="border border-green-600 rounded w-full py-2 px-3  leading-tight focus:outline-none text-shadow-black"
                               required />
                        <!----DNI------------------------------------------------->
                        <label for="dni" class="block text-green-600 text-lg font-bold mb-2 mt-4">DNI :</label>
                        <input type="text" id="dni" name="usuario" placeholder="DNI" value="<c:if test="${not empty datosFormulario['dni']}"><c:out value="${datosFormulario['dni']}" /></c:if>"
                               class="border border-green-600 rounded w-full py-2 px-3 leading-tight focus:outline-none text-shadow-black"/>
                        <!----Boton------------------------------------------------->
